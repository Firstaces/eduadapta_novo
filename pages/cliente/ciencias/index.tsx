...
export const getStaticProps: GetStaticProps = async () => {
  const root = path.join(process.cwd(), "public", "atividades", "ciencias");
  ...
  safeList(root, "/atividades/ciencias");
  safeList(manifests, "/atividades/ciencias/manifests");

  items.sort((a, b) => a.order - b.order || a.title.localeCompare(b.title));
  return { props: { items } };
};

export default function Ciencias({ items }: { items: Item[] }) {
  return (
    <main style={{ padding: 24, fontFamily: "system-ui" }}>
      ...
      <div style={{ marginTop: 24 }}>
        <a href="/cliente/ciencias/01-interativo">
          Aula Interativa: Ciências (C01)
        </a>
      </div>
